...
function LogData() {
  ...
  const getFileData1 = async () => {
    try {
      ...
      alert(`IP Address is Incorrect or Connection to host(CADA/VPN) is lost or Server is not running`);
    }
  };
  ...
  const Display_time = (event) => {
    ...
    const time_data =daysValue + " days " + hoursValue + " hours " + minutesValue + " mins " + secondsValue + " secs";
    setResult(time_data);
  };
