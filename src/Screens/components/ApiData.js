...
function FetchData() {
    ...
    if (serialNo.startsWith('MB') || serialNo.startsWith('BLE')) {
      try {
        const [response1, response2, response3] = await Promise.all([
          ...
          axios.get(
            `${ipaddress}/getamplifierprofiles?ipaddress=${serialno}&slotno=1`
          ),
          axios.get(
            `${ipaddress}/getactivitylog?ipaddress=${serialno}&slotno=1`
          ),
        ]);
  ...
  const dspowerlevels = async () => {
    try {
      const response = await fetch(
        `${ipaddress}/powerLevelStatus?ipaddress=${serialno}&slotno=1&subslotno=0&stream=DS`
  ...
  const uspowerlevels = async () => {
    try {
      const response = await fetch(
        `${ipaddress}/powerLevelStatus?ipaddress=${serialno}&slotno=1&subslotno=0&stream=US`
      ); // Replace <API_ENDPOINT> with the actual API endpoint
      const jsonData = await response.json();
  ...
  const usSpectrumData = async () => {
    try {
      const response = await fetch(
        `${ipaddress}/getdnspectstatus?ipaddress=${serialno}&slotno=1&subslotno=0&stream=US&strtFreqncy=10`
      ); // Replace <API_ENDPOINT> with the actual API endpoint
      const jsonData = await response.json();
  ...
  const dsSpectrumData = async () => {
    try {
      const response = await fetch(
        `${ipaddress}/getdnspectstatus?ipaddress=${serialno}&slotno=1&subslotno=0&stream=DS&strtFreqncy=10`
      ); // Replace <API_ENDPOINT> with the actual API endpoint
      const jsonData = await response.json();
      const extractedData1 = jsonData[0];
  ...
  const dsInputSpectrumData = async () => {
    try {
      const response = await fetch(
        `${ipaddress}/getdnspectstatus?ipaddress=${serialno}&slotno=1&subslotno=0&stream=DSIN&strtFreqncy=10`
      ); // Replace <API_ENDPOINT> with the actual API endpoint
      const jsonData = await response.json();
      const extractedData1 = jsonData[0];
  ...
  const handleSubmit = (event) => {
    ...
    if (ipAddress && serialNo) {
      console.log(
        `Submitting form with IP address: ${ipAddress} and serial number: ${serialNo}`
      );
    }
  };
  ...
  const signalProperties = data9.signals
    ? Object.entries(data9.signals[0])
    : [];
  const signalProperties1 = data9.Splmnufsignals
    ? Object.entries(data9.Splmnufsignals[4])
    : [];
  const signalProperties2 = data9.Spldbgsignals
    ? Object.entries(data9.Spldbgsignals[8])
    : [];
    ...
    if (Object.keys(data9).length > 0) {
      return dtTables.map((tableKey) => (
        <tr key={tableKey}>
          ...
          {data9[tableKey] &&
            Object.entries(data9[tableKey]).map(([key, value]) => (
              <tr key={key}>
                <td>{key}:</td>
                <td>{value}</td>
              </tr>
    ...
    <div>
      ...
      <button  >
        ...
        pData">SNMP Data </Link>
          ...
          <span style={{ color: 'green', fontSize: '12px', marginLeft: '20%' }}>
            {formattedTimestamp}
          </span>
          {isVisible &&
            (serialNo.startsWith('MB') || serialNo.startsWith('BLE')) && (
              <div style={{ marginLeft: '350px' }}>
                <button type="submit" onClick={dsSpectrumData}>
                  DS Spectrum Data
                </button>
                <button type="submit" onClick={dsInputSpectrumData}>
                  DS Input Spectrum Data
                </button>
                <button type="submit" onClick={usSpectrumData}>
                  US Spectrum Data
                </button>
                <button type="submit" onClick={dspowerlevels}>
                  DS Power Levels
                </button>
                <button type="submit" onClick={uspowerlevels}>
                  US Power Levels
                </button>{' '}
              </div>
            )}
        ...
        {isVisible && serialNo.startsWith('OA') && (
          ...
          >
            <table>
              <tr style={{ border: '1px solid black' }}>
                ...
                <td style={{ textAlign: 'left', verticalAlign: 'top' }}>
                  <b style={{ textAlign: 'center' }}>(0x2C Info)</b>
                  <br />
                  {Object.keys(data9).length > 0 &&
                    Object.keys(data9['0x2C']).map((key) => (
                      <tr key={key}>
                        <td colSpan="12" align="left">
                          {key}
                        </td>
                ...
                <td style={{ textAlign: 'left', verticalAlign: 'top' }}>
                  ...
                  <br />
                  {Object.keys(data9).length > 0 &&
                    Object.keys(data9['Node_Info']).map((key) => (
                      <tr key={key}>
                        <td colSpan="12" align="left">
                          {key}
                        </td>
                ...
                <td style={{ textAlign: 'left', verticalAlign: 'top' }}>
                  ...
                  <br />
                  {Object.keys(data9).length > 0 &&
                    Object.keys(data9['Mcu_Info']).map((key) => (
                      <tr key={key}>
                        <td colSpan="12" align="left">
                          {key}
                        </td>
                ...
                <td style={{ textAlign: 'left', verticalAlign: 'top' }}>
                  <b>Advanced Diagnostics(0x28)</b>
                  <br />
                  {Object.keys(data9).length > 0 &&
                    Object.keys(data9['Ingress_Info']).map((key) => (
                      <tr key={key}>
                        <td colSpan="12" align="left">
                          {key}
                        </td>
            ...
            <OATable
              property={signalProperties}
              data={data9.signals}
              title={'I2C Signals'}
            />
            <OATable
              property={signalProperties1}
              data={data9.Splmnufsignals}
              title={'Manufacturing Signals'}
            />
            <OATable
              property={signalProperties2}
              data={data9.Spldbgsignals}
              title={'Debug Signals'}
            />
          </div>
        )}
            ...
            <div style={{ position: 'relative', top: '55px' }}>
              {Object.keys(data).length > 0 && (
                <div
                  ...
                  style={{ alignItems: 'baseline', border: '1px solid black' }}
                >
                  {
                    <div
                      className={styles.tables1}
                      style={{
                        alignItems: 'baseline, border:"1px solid black',
                      }}
                    >
...

              {
                ...
                >
                  <SpectrumTable
                    title={'Downstream Output Graph'}
                    data={data5}
                  />
                  <SpectrumTable
                    title={'Downstream Input Graph'}
                    data={data12}
                  />
