...
const splitBytetoBit = (splitdata, row) => {
 ...
 return binaryValue[row.extraTextBoxes[0]] === "1" ? row.extraTextBoxes[2] : row.extraTextBoxes[1];
};
...
function convert6Values(splitdata, row) {
  const data1 = splitdata.split(" ");
console.log("Convert 6 values"+data1)
  if (data1[0] === '00') {
    return row.extraTextBoxes[0];
  }
  if (data1[0] === '01') {
...
function convert7Values(splitdata, row){
  ...
  useEffect(() => {
    console.log(result); // Log the updated result after it changes
  }, [result]);
  ...
  const handleInputChange1 = (e, index = null) => {
    const { name, value } = e.target;
    if (index !== null) {
      // Updating extraTextBoxes array
      const updatedExtraTextBoxes = [...formData.extraTextBoxes];
      updatedExtraTextBoxes[index] = value;
      setFormData((prevData) => ({ ...prevData, extraTextBoxes: updatedExtraTextBoxes }));
    } else {
      ...
      {rows.map((row, index) => (
        ...
        >
          <input
            ...
            value={row.varByte}
            onChange={(e) => handleInputChange(index, "varByte", e.target.value)}
            style={{
              flex: 1,
              padding: "5px",
              fontSize: "12px",
          ...
          {row.extraTextBoxes.map((text, textBoxIndex) => (
            <input
              ...
              onChange={(e) => {
                const updatedRows = [...rows];
                updatedRows[index].extraTextBoxes[textBoxIndex] = e.target.value;
                setRows(updatedRows);
              }}
              style={{
                flex: 1,
          ...
          <input
            ...
            value={row.startByte}
            onChange={(e) => handleInputChange(index, "startByte", e.target.value)}
            style={{
              flex: 1,
              padding: "5px",
              fontSize: "12px",
          ...
          <input
            ...
            value={row.endByte}
            onChange={(e) => handleInputChange(index, "endByte", e.target.value)}
            style={{
              flex: 1,
              padding: "5px",
              fontSize: "12px",
            ...
            {result.map((row, index) => (
              ...
              >
                <span style={{ ...styles.item, color: "rgb(0, 116, 232)" }}>
                  {row.varByte +" ("+ row.startByte + "..." +row.endByte+" bytes"+")"}
                </span>
